								...
								<div class="table-responsive">
									<table class="table table-sm">
										...
										<tbody>
											...
											{foreachelse}
											<tr>
												<td class="total-pay-row text-center" colspan="4">{lang key='invoicestransnonefound'}</td>
											</tr>
											{/foreach}
											<tr>
					...
					<div class="col-lg-3 col-md-5 col-12 view__invoice__page__sidebar__wrapper position-relative">
						...
						{if $status eq "Unpaid" && $allowchangegateway}
							...
							{if $manualapplycredit}
								<div class="view__invoice__page__sidebar__add-credit mt-5">
									<div class="card">
										<div class="card__header py-4">
											<h6 class="coodiv-text-9 mb-0">{lang key='invoiceaddcreditapply'}</h6>
										</div>
										<div class="card__body">
											<form method="post" action="{$smarty.server.PHP_SELF}?id={$invoiceid}">
												<input type="hidden" name="applycredit" value="true" />
												<p class="coodiv-text-12 mb-5 font-weight-400 d-block">{lang key='invoiceaddcreditdesc1'} <strong>{$totalcredit}</strong>. {lang key='invoiceaddcreditdesc2'}. {lang key='invoiceaddcreditamount'}:</p>
												<div class="input-group">
													<input type="text" name="creditamount" value="{$creditamount}" class="form-control from-control-sm" />
													<div class="input-group-append">
														<button type="submit" class="btn btn-sm btn-success" id="btnInvoiceAddCreditApply">
