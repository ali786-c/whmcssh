            ...
            <div class="view__ticket__wrapper__item">
                <div class="ticket-reply markdown-content{if $reply.admin} staff{/if}">
                    ...
                    <div class="view__ticket__wrapper__item__message">
						...
						<div class="view__ticket__wrapper__item__message__inner__preivew">
							{$reply.message}
						</div>
...
(function () {
    "use strict";
    var MESSAGE_SELECTOR = ".view__ticket__wrapper__item__message__inner__preivew";
    var SPACER_HEIGHT = "5px";
    function hasContentAfter(br) {
        ...
        while (node) {
            ...
            if (
                node.nodeType === 1 &&
                !node.hasAttribute("data-ticket-br-spacer") &&
    ...
    function createSpacer() {
        ...
        spacer.setAttribute("data-ticket-br-spacer", "true");
    ...
    function processMessage(message) {
        if (!message) return;
        message.querySelectorAll('br[data-br-spacer="true"], span[data-small-br-spacer="true"]').forEach(function (oldSpacer) {
            oldSpacer.remove();
        });

        var brTags = message.querySelectorAll("br");

        brTags.forEach(function (br) {
            if (br.closest("pre, code")) {
                return;
            }
            if (!hasContentAfter(br)) {
                return;
            }
            if (
                br.nextSibling &&
                br.nextSibling.nodeType === 1 &&
                br.nextSibling.hasAttribute("data-ticket-br-spacer")
            ) {
                br.nextSibling.style.height = SPACER_HEIGHT;
                return;
            }

            br.parentNode.insertBefore(createSpacer(), br.nextSibling);
        });
    }

    function processAllMessages(root) {
        root = root || document;
