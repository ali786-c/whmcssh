...
<form method="post" action="{routePath('password-reset-validate-email')}" role="form">
   ...
   {if $captcha && $captcha->isEnabled() && $showCaptchaAfterLimit}
    ...
    <div class="form-group text-center mb-0">
        <button id="resetPasswordButton" {if $showCaptchaAfterLimit}data-captcha-required="true"{/if} type="submit" class="btn btn-primary{$captcha->getButtonClass($captchaForm)} d-block w-100">
