...
<div class="{$coodivsettings.loginstyle|default:''} main__login__content__wrapper primary-content">
			...
			<div class="col-md-8 col-12">
				<div class="card">
					...
					<div class="card__body text-center">
						<p class="coodiv-text-10 font-weight-400">{lang key='verifyEmailAddress'}</p>
						<div class="d-flex align-items-center justify-content-center gap-5 flex-wrap mt-5">
						<button id="btnResendVerificationEmail" class="btn btn-sm btn-primary btn-resend-verify-email btn-action" data-email-sent="{lang key='emailSent'}" data-error-msg="{lang key='error'}" data-uri="{routePath('user-email-verification-resend')}">
						<span class="loader w-hidden"><i class="fa fa-spinner fa-spin"></i></span>
						{lang key='resendEmail'}
						</button>	
						<a href="{$WEB_ROOT}/logout.php" class="btn btn-sm btn-light">{lang key='logouttitle'}</a>
						</div>
					</div>
				</div>
			</div>
	    </div>
		<h6 class="the__copyright__text coodiv-text-12 mb-0 font-weight-500 text-center">{lang key="copyrightFooterNotice" year=$date_year company=$companyname}.</h6>		
	</div>
</div>
{else}
<script type="text/javascript">
	setTimeout(function() {
		window.location.href = (whmcsBaseUrl + '/clientarea.php')	
	}, 1);
</script>
{/if}
