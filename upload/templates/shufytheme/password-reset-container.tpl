			...
			<div class="default__shadow__panel__body">
				...
				{else}
					{if $successMessage}
						...
						<i class="form__check__successmessage__icon fal fa-check"></i>
						<h6 class="coodiv-text-6 font-weight-bold mb-0 text-center">{lang key='pwresetrequested'}</h6>
						<p class="coodiv-text-10 font-weight-400 text-center">{$successMessage}</p>
						</div>
					{else}
						{if $errorMessage}
							{include file="$template/includes/alert.tpl" type="error" title="{lang key='clientareaerrors'}" msg=$errorMessage}
						{/if}
						{if $innerTemplate}
