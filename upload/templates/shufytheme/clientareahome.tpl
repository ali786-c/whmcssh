{assign var=hiddencardsclientarea value=['']}
{if $coodivsettings.clientareaservices|default:'' != 'activated' && $coodivsettings.id|default:'' == '1'}{$hiddencardsclientarea[] = 'servicesPanel'}{/if}
{if $coodivsettings.clientareatickets|default:'' != 'activated' && $coodivsettings.id|default:'' == '1'}{$hiddencardsclientarea[] = 'ticketsPanel'}{/if}
{if $coodivsettings.clientareadomains|default:'' != 'activated' && $coodivsettings.id|default:'' == '1'}{$hiddencardsclientarea[] = 'registerDomainPanel'}{/if}
{if $coodivsettings.clientareaannouncements|default:'' != 'activated' && $coodivsettings.id|default:'' == '1'}{$hiddencardsclientarea[] = 'announcementsPanel'}{/if}
...
{function name=outputHomePanels}	
	{if !in_array($item->getAttribute('id'), $hiddencardsclientarea) && !in_array($item->getName(), $hiddenfromclientarea) && !in_array($item->getAttribute('id'), $alertsfromclientarea)}
		<div menuItemName="{$item->getName()}" class="card {if $item->getClass()}{$item->getClass()}{/if}" {if $item->getAttribute('id')} id="{$item->getAttribute('id')}"{/if}>
			...
			{/if}
			{if $item->hasChildren()}
				<div id="{if $item->getAttribute('id') == "servicesPanel"}list-container-1{else if $item->getAttribute('id') == "ticketsPanel"}list-container-2{/if}" class="body__list__group__wrapper {if $item->getChildrenAttribute('class')}{$item->getChildrenAttribute('class')}{/if}">
					<div id="{if $item->getAttribute('id') == "servicesPanel"}list-1{else if $item->getAttribute('id') == "ticketsPanel"}list-2{/if}">
						{foreach $item->getChildren() as $childItem}
							{if $childItem->getUri()}
								<a menuItemName="{$childItem->getName()}" href="{$childItem->getUri()}" class="item body__list__group__item body__list__group__item__link {if $childItem->getClass()} {$childItem->getClass()}{/if}{if $childItem->isCurrent()} active{/if}"{if $childItem->getAttribute('dataToggleTab')} data-toggle="tab"{/if}{if $childItem->getAttribute('target')} target="{$childItem->getAttribute('target')}"{/if} id="{$childItem->getId()}">
							...
							{else}
								<div menuItemName="{$childItem->getName()}" class="item body__list__group__item {if $childItem->getClass()} {$childItem->getClass()}{/if}" id="{$childItem->getId()}">
									{if $childItem->hasIcon()}<i class="{$childItem->getIcon()}"></i>&nbsp;{/if}
									{$childItem->getLabel()}
									{if $childItem->hasBadge()}&nbsp;<span class="badge">{$childItem->getBadge()}</span>{/if}
								</div>
			...
			{if $item->getAttribute('id') == "servicesPanel" || $item->getAttribute('id') == "ticketsPanel"}
				<div class="card__footer card__pagination__wrapper mt-auto">
					<div class="card__pagination__wrapper__item clientareahome__pagination" id="{if $item->getAttribute('id') == "servicesPanel"}pagination-1{else if $item->getAttribute('id') == "ticketsPanel"}pagination-2{/if}">
					  <button class="prev btn btn-light btn-sm">{$LANG.tablepagesprevious}</button>
					  <span class="page-numbers"></span>
					  <button class="next btn btn-light btn-sm">{$LANG.tablepagesnext}</button>
					</div>
				</div>
			{else}
...
{if $coodivsettings.clientareamaininformations|default:''=='activated' || $coodivsettings.id|default:'' != '1'}
				...
				<div class="d-flex flex-wrap flex-sm-nowrap mb-3">
					<div class="mr-7 mb-4 d-md-flex d-none">
						<div class="client__area__profile__pic">
							{if $coodivsettings.gravatar|default:''=='activated' || $coodivsettings.id|default:'' != '1'}
						...
						<div class="d-flex justify-content-between align-items-start flex-wrap mb-2">
							<div class="d-flex flex-column">
								...
								<div class="d-flex flex-wrap fw-semibold fs-6 mb-4 pe-2">
									{if $coodivsettings.customerspin|default:''=='activated'}<span class="d-flex align-items-center text-muted mr-5 mb-2 coodiv-text-12 font-weight-600"><i class="fad fa-user-circle mr-2"></i> {$coodivgeneratePinCode}</span>{/if}
...
{/if}
{if $coodivsettings.accountoverview|default:''=='activated' || $coodivsettings.id|default:'' != '1'}
...
<div class="client__area__services with__sidebar">
	<div class="row justify-content-start">
		 <div class="client-home-cards {if $coodivsettings.clientmarketconnect|default:''=='activated'|| $coodivsettings.id|default:'' != '1'}{if $CoodivMarketConnectServices}col-md-8{/if}{/if} col-12">
			{foreach $panels as $item}
				{outputHomePanels}
            {/foreach}
		 </div>
		 {if $coodivsettings.clientmarketconnect|default:''=='activated' || $coodivsettings.id|default:'' != '1'}
...
<script>
function createPagination(listContainerId, listId, paginationId) {
  ...
  function createPagination() {
    var totalPages = Math.ceil(list.getElementsByClassName('item').length / itemsPerPage);
  ...
  function updatePagination() {
    ...
    var pageLinks = pageNumbers.getElementsByTagName('a');
    for (var i = 0; i < pageLinks.length; i++) {
      if (parseInt(pageLinks[i].innerHTML) === currentPage) {
...
if(jQuery("#list-container-1").length > 0) {
createPagination('list-container-1', 'list-1', 'pagination-1');
}
if(jQuery("#list-container-2").length > 0) {
createPagination('list-container-2', 'list-2', 'pagination-2');
}
</script>
