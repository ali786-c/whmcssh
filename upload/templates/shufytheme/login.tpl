<div class="{$coodivsettings.loginstyle|default:''} main__login__content__wrapper">
	<div class="main__login__content">
	
	<div class="autho__logo__wrapper website__identity__sidebar {if $coodivsettings.loginstyle|default:''!='loginstyletwo'}d-lg-none{/if} d-flex align-items-center justify-content-center mb-8">		
	...
	<form method="post" action="{routePath('login-validate')}" class="login-form" role="form">
		<div class="{if $coodivsettings.loginstyle|default:''=='loginstyleone' || $coodivsettings.id|default:'' != '1'}panel__without__shadow{/if} default__shadow__panel">
			<div class="default__shadow__panel__body auth__page__inner__wrapper">
				<div class="mb-8 text-center">
					...
					<p class="coodiv-text-10 font-weight-400">{lang key='userLogin.signInToContinue'}</p>
				</div>
				...
				<div class="d-block mb-4">
					<button id="login" type="submit" class="btn btn-primary{$captcha->getButtonClass($captchaForm)} d-block w-100">
						{lang key='loginbutton'}
					</button>
				</div>
			</div>
			<div class="default__shadow__panel__footer">
				<p class="coodiv-text-11 mb-0 font-weight-400 text-center">{lang key='userLogin.notRegistered'} <a href="{$WEB_ROOT}/register.php">{lang key='userLogin.createAccount'}</a></p>
			</div>
		</div>
	</form>
	{if $coodivsettings.loginformlanguage|default:''=='activated' || $coodivsettings.id|default:'' != '1'}
