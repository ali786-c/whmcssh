			...
			<div class="pricing__plans__standard__item">
				<div class="pricing__plans__standard rounded-8 pt-12 pb-8 px-9 white-bg h-100 d-flex flex-column mx-3 {if $product.isFeatured}featured__product position-relative{/if}">
					...
					<p class="coodiv-text-11 font-weight-300">
						{if $product.custom_tagline}
						{$product.custom_tagline}
						{else}
						{$groupname}
						{/if}
					</p>
					<p class="coodiv-text-11 font-weight-400 mt-8 main-text-color">
						{if $product.custom_short_description}
					...
					<div class="buttom-pricing-element">
						<div class="pricing d-flex align-items-start mt-lg-10 mt-4 mb-2">
							...
							{if $product.pricing.minprice.cycle eq "monthly"}
								{if $product.pricing.rawpricing.monthly > 0}
								  	<span data-row-price="{$product.pricing.rawpricing.monthly}">{$product.pricing.rawpricing.monthly|number_format:2:'.':','}</span>
								{else}
									<span data-row-price="{$minimalprice}">{$minimalprice|number_format:2:'.':','}</span>
								{/if}		
								{elseif $product.pricing.minprice.cycle eq "quarterly"}
							<span id="quarterlyprice" data-row-price="{$product.pricing.rawpricing.quarterly}">{$product.pricing.rawpricing.quarterly|number_format:2:'.':','}</span>
								{elseif $product.pricing.minprice.cycle eq "semiannually"}
							<span id="semiannuallyprice" data-row-price="{$product.pricing.rawpricing.semiannually}">{$product.pricing.rawpricing.semiannually|number_format:2:'.':','}</span>
								{elseif $product.pricing.minprice.cycle eq "annually"}
							<span id="annuallyprice" data-row-price="{$product.pricing.rawpricing.annually}">{$product.pricing.rawpricing.annually|number_format:2:'.':','}</span>
								{elseif $product.pricing.minprice.cycle eq "biennially"}
							<span id="bienniallyprice" data-row-price="{$product.pricing.rawpricing.biennially}">{$product.pricing.rawpricing.biennially|number_format:2:'.':','}</span>
								{elseif $product.pricing.minprice.cycle eq "triennially"}
							<span id="trienniallyprice" data-row-price="{$product.pricing.rawpricing.triennially}">{$product.pricing.rawpricing.triennially|number_format:2:'.':','}</span>
								{elseif $product.pricing.minprice.cycle eq "free"}
								{$LANG.orderfree}
							{else}
							{$product.pricing.rawpricing.monthly|number_format:2:'.':','}
							{/if}
							</span>
							{/if}
							</span>
