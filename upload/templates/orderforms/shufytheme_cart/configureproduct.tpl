                ...
                <div class="row justify-content-start">
                    <div class="secondary-cart-body">
												...
												<div class="default__shadow__panel__body__small">
													<label class="radio cart__choose__cycle__label d-flex justify-content-center {if $billingcycle eq "monthly"} checked{/if}" for="monthly" {if $configurableoptions}data-update-config data-config-i={$i} data-config-val="monthly"{else}data-change-billingcycle{/if}>
														...
														<div class="cart__choose__cycle__content text-center">
															...
															<p class="coodiv-text-6 main-text-color font-weight-bold">{$currency.prefix}{$pricing.rawpricing.monthly|number_format:2:'.':','}{$currency.suffix}</p>
															<span class="coodiv-text-12 font-weight-300">{$currency.prefix}<span>{$pricing.rawpricing.monthly|number_format:2:'.':','}</span> {$LANG.orderpaymenttermmonthly}</span>
														</div>
													</label>
												</div>
											</div>
                        ...
                        {if $configurableoptions}
                           <div class="product__config__loader section__loader">
                                ...
                                <div class="d-flex flex-column">
                                    ...
                                    <div class="configoption__optiontype__2 mb-6">
										...
										<div class="panel__check__cart__group row justify-content-start">
											...
											<div class="item__grid col-md-4 col-6">
												<div class="radio__check__panel default__shadow__panel">
													<div class="default__shadow__panel__body__small logo__based__configoption option-{$options.id}-config">
														<label class="panel__check__cart__group__label d-flex justify-content-start" for="{$options.id}">
															<div class="cercle__input__radio input">
																<input class="configoption__radio__input" type="radio" name="configoption[{$configoption.id}]" value="{$options.id}"{if $configoption.selectedvalue eq $options.id} checked="checked"{/if} />
																<div class="icheck__control__check__icon">
																	<div class="icheck__control__check__icon__checked__effect"></div>
																</div>
															</div>
															<div class="panel__check__cart__group__content">
																<h6 class="coodiv-text-10 panel__check__cart__group__title">{if $options.nameonly}{$options.nameonly}{else}{$LANG.enable}{/if}</h6>
																<p class="coodiv-text-12 font-weight-600 panel__check__cart__group__pricing">   
																	...
																	{else}
																		<span class="panel__check__cart__group__pricing__tag">{$currency.prefix}{$options.fullprice}{$currency.suffix}</span>
																	{/if}
...
$(document).on('change', 'input[name="billingcycle"]', function () {
    ...
    setTimeout(function () {
        if (typeof updateConfigurableOptions === 'function') {
            $('.product-configurable-option').each(function (index) {
                updateConfigurableOptions(index, selectedCycle);
            });
        }
    }, 100);
});
...
$(document).on('change', '.configoption__radio__input', function () {
